...
TEST(symbol, terminal_to_string) {
  ...
  EXPECT_EQ("Operand[whatever]", operand_ptr->ToString());
}
...
TEST(symbol, non_terminal_to_string) {
  ...
  logic_and_ptr->children_.push_back(operand_ptr);
  EXPECT_EQ("LogicAnd(Operand[whatever])", logic_and_ptr->ToString());
  // more than one.
  auto posix_option_ptr = PosixOption::Init(t_posix_option);
  logic_and_ptr->children_.push_back(posix_option_ptr);
  EXPECT_EQ(
      "LogicAnd(Operand[whatever], PosixOption[-c])",
